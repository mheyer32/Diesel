...
/*	Lexer is a utility to parse ASCII files.
    It does not divide the text into tokens. Instead, the user has to explicitely parse the text in a straight-forward
   manner. Therefore Lexer should be only used in files that do not contain a deep hierarchy, like c++ source or
   similar. Lexer will automatically skip C and C++ comments and other whitespaces.
*/
...
protected:
    ...
    public:
        ...
        : val(0)
        , maxval(0) {};
        inline maxint(int value, int maxvalue)
        : val(value)
        , maxval(maxvalue)
        {
            check();
        ...
        inline maxint& operator=(int newval)
        {
            val = newval;
            check();
            return *this;
        }
        inline maxint& operator=(const maxint& mint2)
        {
            val    = mint2.val;
            maxval = mint2.maxval;
            return *this;
        };
        inline maxint& operator++()
        {
            val++;
            check();
            return *this;
        };
        inline maxint operator++(int)
        {
            maxint temp = *this;
            ++(*this);
            check();
        ...
        inline maxint& operator+=(int val2)
        {
            val += val2;
            check();
            return *this;
