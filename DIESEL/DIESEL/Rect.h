...
struct RECTANGLE
{
	...
	RECTANGLE():min(VECTOR2(999999,999999)),max(VECTOR2(-999999,-999999)) {};
	RECTANGLE( const VECTOR2 &nmin, const VECTOR2 &nmax):min(nmin),max(nmax) {};
...
{
	if (rect2.min.x < min.x) min.x=rect2.min.x;
	if (rect2.min.y < min.y) min.y=rect2.min.y;
	if (rect2.max.x > max.x) max.x=rect2.max.x;
	if (rect2.max.y > max.y) max.y=rect2.max.y;
