...
struct PORTAL
{
	...
	PORTAL():num_vertices(0),vertices(NULL),markportal(0){};
	~PORTAL()
	{
		delete [] vertices;
	};
...
struct AREA
{
	...
	void makeNewFrustum(const VECTOR3 &origin, const VECTOR3 *portalpoints, int num_points, Frustum &frustum,int otherside);
