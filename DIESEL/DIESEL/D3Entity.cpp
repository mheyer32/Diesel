...
{
	VECTOR3 vec;
	int num_args=sscanf(text.c_str(),"%f %f %f",&vec.x,&vec.z,&vec.y); // automatically convert from doom to std format
	if (num_args!=3) cout<<"WARNING: D3Entity::getVec3() error while parsing vector in entity: "<<getName()<<endl;
	vec.z=-vec.z;
	return vec;
}
...
MATRIX3 D3Entity::getRotation(const std::string &text)
{
	MATRIX3 mat;
	// row-major format
	int num_args=sscanf(text.c_str(),"%f %f %f %f %f %f %f %f %f",&mat.m00,&mat.m01,&mat.m02,&mat.m10,&mat.m11,&mat.m12,&mat.m20,&mat.m21,&mat.m22);
	//int num_args=sscanf(text.c_str(),"%f %f %f %f %f %f %f %f %f",&mat.m00,&mat.m10,&mat.m20,&mat.m01,&mat.m11,&mat.m21,&mat.m02,&mat.m12,&mat.m22);

	if (num_args!=9) cout<<"WARNING: D3Entity::getRotation() error while parsing rotation in entity: "<<getName()<<endl;

	// convert desired rotation to our coordinate system
	MATRIX3 mat2;
	mat2[0]=1;mat2[3]=0; mat2[6]=0;
	mat2[1]=0;mat2[4]=0; mat2[7]=1;
	mat2[2]=0;mat2[5]=-1; mat2[8]=0;
