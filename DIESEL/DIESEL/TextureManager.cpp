...
CTexture* CTextureManager::findTexture(const std::string &name)
{
	...
	{
		if (lowname==(*tIt)->getName()) return *tIt;
	}
...
CVideoTexture* CTextureManager::findOrLoadVideoTexture(const std::string &name)
{
	...
	if (vidtex=dynamic_cast<CVideoTexture*>(tex)) return vidtex;	//FIXME: check for right type
	...
	{
		std::cout<<"CTextureManager::findOrLoadVideoTexture(): unsupported videoformat:"<<videofile.getExtension()<<endl;
		return NULL;
	}
...
void CTextureManager::restoreAllTextures()
{
	...
	for (;tIt!=registeredtextures.end();++tIt)
	{
		...
		{
			std::cout<<"CTextureManager::restoreAllTextures(): could not restore texture: "<<(*tIt)->getName()<<"Filename: "<<(*tIt)->getFilename()<<std::endl;
			//throw CException(string("CTexture::restoreAllTextures() could not restore texture: ")+(*tIt)->getName()+string(" Filename: ")+(*tIt)->getFilename());
		}
	}
}
