...
void CShaderPass::addTexture(CTexture* texture)
{
	...
	if (anim_numframes>1) passflags|=PASS_ANIMMAP;
...
void CShaderPass::setBlendFunc(int src, int dest)
{
	...
	if ((src==GL_ZERO && dest==GL_SRC_COLOR)||
		(src==GL_DST_COLOR && dest==GL_ZERO))
	{
	...
	if (src==GL_SRC_ALPHA && dest==GL_ONE_MINUS_SRC_ALPHA)
	{
		passflags|=PASS_TEXENV;
		texenv=GL_DECAL;
	}
...
{
	if (write)	passflags|=PASS_DEPTHWRITE;
	else passflags&=~PASS_DEPTHWRITE;
}
void CShaderPass::setDepthTest(bool test)
{
	if (test)	passflags&=~PASS_NODEPTHTEST;
	else passflags|=PASS_NODEPTHTEST;
}
...
{
	if (use_lighting) passflags|=PASS_LIGHTING;
	else passflags&=~PASS_LIGHTING;
