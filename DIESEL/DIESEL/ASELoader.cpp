...
void ASELoader::readMaterialList()
{
	...
	for (int m=0; m<num_materials; ++m)
	{
		...
		if (shader)	materials[m]=shader;
		else materials[m]=CShader::DefaultShader;
	}
...
CVertexBuffer* ASELoader::buildVBuffer(VECTOR3 *verts, VECTOR2* tverts, INDEX *faces, INDEX* tfaces, int num_faces, int num_verts)
{
	assert(verts);
	assert(tverts);
	assert(faces);
	...
	for (int v=0; v<num_indices; ++v) remap[v]=false;
	for (int t=0; t<num_indices; ++t)
	{
		if (remap[t]) continue; //index is already remapped
		remap[t]=true;
		const VECTOR2 &v1= tverts[tfaces[t]];
		for (int t2=t+1; t2<num_indices; t2++)
		{
			if (remap[t2]) continue;
	...
	CVertexBuffer *vbuffer=new CVertexBuffer(MT_STATIC,VERTEXARRAY|INDEXARRAY|TEXCOORDARRAY,num_finalverts,num_faces*3);
