...
bool LWOLoader::LoadLWO(const CPath &Filename, CEntity::ENTITYLIST &EntityList)
{
	...
	type=readInt();
	if (type==FACE)	readFaces(vbuffer);
	...
	if (!seekChunk(VMAD))
	{
		...
		{
			type=readInt();
			if (type==TXUV) readTexCoords(vbuffer);
		}
	}
	else
	{
...
void LWOLoader::readTexCoords(CVertexBuffer *vbuffer)
{
	...
	if (!(strlen(temp)&1)) file->readBYTE(); // skip pad byte if necessary
...
bool LWOLoader::skipChunk()
{
	...
	if (current_chunk.pos<current_chunk.maxseek) readChunk(); // falls ExitSubChunk ganz am Ende des Files aufgerufen wird
	return true;
}
