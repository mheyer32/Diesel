...
CWave::CWave(void):waveform(WF_SIN), wavefunc(&CWave::sin), base(0.0f), amplitude (0.0f), freq(0.0f)
{
...
{
	if (EQ(wavename,"sin")) return WF_SIN;
	else if (EQ(wavename,"square")) return WF_SQUARE;
	else if (EQ(wavename,"sawtooth")) return WF_SAWTOOTH;
	else if (EQ(wavename,"inversesawtooth")) return WF_INVERSESAWTOOTH;
	else if (EQ(wavename,"triangle")) return WF_TRIANGLE;
	else if (EQ(wavename,"noise")) return WF_NOISE;
