...
Frustum::Frustum(int size) : m_planes(size)
{
	if (size==0) m_planes.reserve(6);
 ...
 bool Frustum::BoxInFrustum(const BBOX &bbox) const
{
	...
	{
		if (BoxBehind(bbox,m_planes[p])) return false;
	}
	return true;
}
 ...
 int Frustum::BoxOnFrustumSide(const BBOX &bbox) const
{
	...
	for (std::vector<PLANE>::size_type p=0; p<size; ++p)
	{
		...
		allsides|=side;
		if (side==BACKSIDE) return side;
		if (allsides==SPANNING) return allsides;
	}
	return allsides;
}
...
bool Frustum::PointInFrustum(const VECTOR3 &point) const
{
	...
	for (std::vector<PLANE>::size_type p=0; p<size; ++p)
	{
		if (PointOnPlaneSide(point,m_planes[p])==BACKSIDE) return false;
	}
	return true;
}
/*
