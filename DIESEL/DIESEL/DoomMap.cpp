...
void DoomMap::cullMap()
{
	...
	if ((int)r_showarea && current_area!=areanum) cout<<"Camera in Area: "<<areanum<<endl;
...
int	DoomMap::getArea(const VECTOR3 &position)
{
	...
	do
	{
		node = (nodes[node].plane.n*position > nodes[node].plane.d) ? nodes[node].children[0] : nodes[node].children[1];
	} while(node>0);
...
bool DoomMap::portalVisible(const PORTAL &portal,const Frustum &frustum)
{
	...
	for (std::vector<PLANE>::size_type p=0; p<size; ++p)
	{
		...
		if (side==BACKSIDE) return false;
	}
	return true;
}

void DoomMap::makeNewFrustum(const VECTOR3 &origin, const VECTOR3 *portalpoints, int num_points, Frustum &newfrustum, int otherside)
{
...
void DoomMap::renderArea_r(int areanum, const Frustum &frustum, int depth)
{
	...
	for (; iIt!=area.portals.end(); ++iIt)
	{
		...
		if (portal.markportal) continue; // don´t walk this portal twice on the current path through the area-portal-area graph
		...
		//if (fabs(PointDistance(g_Camera->getTransformation().getTranslation(),portal.plane))>0.1)
		{
			...
			{
				if (!portalVisible(portal,frustum)) continue;
			}
		}

		portal.markportal=1; // we walk this portal from current area, but don´t do this twice

		if ((int)r_showportals)showportal(inverts,num_inverts);
...
void DoomMap::getTouchedAreas(const BBOX &box, INDEXLIST &arealist, int node)
{
	...
	if (((side&FRONTSIDE)||side==ON_PLANE) && nodes[node].children[0]) getTouchedAreas(box,arealist,nodes[node].children[0]);
	if (((side&BACKSIDE)||side==ON_PLANE) && nodes[node].children[1]) getTouchedAreas(box,arealist,nodes[node].children[1]);
}
