...
//	SetCurrentDirectory("g:\\quake3"); // fuers Profiling
	...
	if(!ogl->InitOpenGL(dummywindow,0,0,false,format)) throw CException("COpenGL::InitOpenGL() creating dummy-context failed");
	ogl->ShutDownOpenGL();

	dummywindow->close();
	KILLOBJECT(dummywindow);

	if(!ogl->InitOpenGL(CAppWindow::Instance(),format)) throw CException("COpenGL()::InitOpenGL() failed");
...
void LoadModel(CPath file, 	CEntity::ENTITYLIST &entitylist)
{
	...
	if (file.getString().empty())
	{
		...
		{
			ogl->SwitchToFullscreen(); // wont work, because the filerequester already switched the window to windowed-mode
		}
		file=filename;
	}
...
void mapCommand(ConCmd &command,const std::string &args)
{
	...
	if (q3scene)
	{
		...
		{
			if ((*eIt)->getClassName()=="info_player_deathmatch"||
				(*eIt)->getClassName()=="info_player_start")
		...
		if (list.size()) g_Camera->getTransformation()=list.front()->getTransformation();
		else g_Camera->getTransformation().setTranslation(NullVector3);
    ...
    for (int y=1; y<width-1; ++y)
    {
		...
		for (int x=1;x<width-1; ++x , ++ax)
		{
			float temp = (((// current_buffer[ax-width-1].y
							...
							+current_buffer[ax+width].y
							//+current_buffer[ax+width+1].y
							+current_buffer[ax-1].y
							+current_buffer[ax+1].y
							)*0.5f)-old_buffer[ax].y);
