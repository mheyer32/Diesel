...
CMesh* MS3DModel::createMesh()
{
    ...
    for (int m = 0; m < m_num_meshes; ++m) {
        ...
        vbuffer->AllocArrays(INDEXARRAY | VERTEXARRAY | NORMALARRAY | TEXCOORDARRAY, msmesh.num_vertices,
                             msmesh.num_indices);
...
void MS3DModel::evaluateRuntimePosture(float frame, CMesh* mesh, MS3DAnimation* anim)
{
    ...
    for (int j = 0; j < m_num_joints; ++j) {
        ...
        if (jointanim.num_poskeys) {
            ...
            VECTOR3 position =
                jointanim.poskeys[frame1].poskey * one_minus_frac + jointanim.poskeys[frame2].poskey * frac;
    ...
    for (int j = 0; j < m_num_joints; ++j) {
        ...
        for (int af = 0; af < num_animframes; ++af) {
            ...
            if (jointkeyframes.num_rotkeys) {
                ...
                slerpQuaternion(frac, jointkeyframes.rotkeys[frame1].rotkey, jointkeyframes.rotkeys[frame2].rotkey,
                                slerp);
            ...
            if (jointkeyframes.num_poskeys) {
                ...
                position += jointkeyframes.poskeys[frame2].poskey * frac +
                            jointkeyframes.poskeys[frame1].poskey * (1.0f - frac);
            }
        }
