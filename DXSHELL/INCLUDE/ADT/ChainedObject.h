...
public:
	...
	static 	_T* findObject(const _KEY& key, _COMP &comp=_COMP())
	{
		...
		while (object)
		{
			if (comp(*object, key)) return object;
...
template <class _T> _T* ChainedObject<_T>::s_firstobject=0;
...
ChainedObject<_T>::ChainedObject(void)
{
	...
	if (s_firstobject)	s_firstobject->m_prevobject=static_cast<_T*>(this);
...
ChainedObject<_T>::~ChainedObject(void)
{
	if (m_nextobject) m_nextobject->m_prevobject=this->m_prevobject;
	if (m_prevobject) m_prevobject->m_nextobject=this->m_nextobject;
	
	if (s_firstobject==static_cast<_T*>(this)) s_firstobject=m_nextobject;
}
