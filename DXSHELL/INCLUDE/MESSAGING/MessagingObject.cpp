...
namespace Msg
{
	...
	{
		if (!s_livingObjects) s_livingObjects=new MSGOBJECTSET;
	...
	void MessagingObject::unregisterListener(MessagingObject *listener, MESSAGEID msgId)
	{
		...
		{
			std::cout<<"MessagingObject::unregisterListener() object tries to unregister message it is not registered for."<<std::endl;
			return;
		}
	...
	void MessagingObject::sendMessage(MESSAGEID msgId, const Param &parameters)
	{
		...
		for (; lIt!=lItend; )
		{
			...
			{
				std::cout<<"WARNING: MessagingObject::sendMessage() listener unknown, probably didn´t unregister properly. unregistering listener automatically."<<std::endl;
				lIt=listeners.erase(lIt);
				continue;
			}
			else
	...
	// default MessageHandler does nothing
	 Msg::MSGRVAL MessagingObject::handleMessage(Msg::MessagingObject* /*sender*/, Msg::MESSAGEID /*msgId*/, const Msg::Param& /*parameters*/)
	 {
		 assert(0);
	 ...
	 Msg::MSGRVAL MessagingObject::sendMessage(Msg::MessagingObject *listener, Msg::MESSAGEID msgId, const Msg::Param &parameters)
	 {
		 assert(exists(listener));
	 ...
	 Msg::MSGRVAL MessageCallback::handleMessage(Msg::MessagingObject *sender, Msg::MESSAGEID msgId, const Msg::Param &parameters)
	 {
